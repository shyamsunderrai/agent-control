...
class ProtectEngine:
    ...
    def _auto_discover_rules(self) -> Optional[Path]:
        """
    ...
    def _fetch_rules_from_server(self) -> None:
        ...
        if not SDK_AVAILABLE:
            print(f"⚠️  Agent Protect SDK not available. Using empty rules.")
            self.rules = {}
            return
        ...
        try:
            ...
            print(f"   Using empty rules. Agent will run without protection.")
            self.rules = {}
    ...
    def get_rule(self, step_id: str) -> Optional[Dict[str, Any]]:
        """
...
_protect_engine: Optional[ProtectEngine] = None
...
def init_protect_engine(rules_file: Union[str, Path]) -> ProtectEngine:
    """
...
def get_protect_engine() -> Optional[ProtectEngine]:
